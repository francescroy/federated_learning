...
model = Sequential([
            ...
            MaxPool2D(pool_size=(2, 2), strides=2),
            ##
            #Conv2D(filters=128, kernel_size=(3, 3), activation='relu', padding='same', kernel_initializer=initializer, bias_initializer=initializer),
            #MaxPool2D(pool_size=(2, 2), strides=2),
            #Conv2D(filters=256, kernel_size=(3, 3), activation='relu', padding='same', kernel_initializer=initializer, bias_initializer=initializer),
            #MaxPool2D(pool_size=(2, 2), strides=2),
            ##
...
for x in range(500):
    ...
    train_batches = image_data_generator.flow_from_directory(
            directory="/home/francesc/Escritorio/datasets/folder_client_1/chest_xray/train",
            target_size=(224, 224),
            classes=['PNEUMONIA', 'NORMAL'],
            batch_size=b_size,
            shuffle=True)
    ...
    valid_batches = image_data_generator.flow_from_directory(
        directory="/home/francesc/Escritorio/datasets/folder_client_1/chest_xray/test",
        target_size=(224, 224),
        classes=['PNEUMONIA', 'NORMAL'],
        batch_size=b_size,
        shuffle=True)
    ...
    model.fit(
            ...
            #validation_steps=25, ### aaaaah okay.
            #verbose=2
          )
